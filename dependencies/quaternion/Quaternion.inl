...
Quaternion
  ::Quaternion(const gpcpu::float4 &v)
    :Parent(v)
{
  ;
} // end Quaternion::Quaternion()
...
Quaternion
  ::Quaternion(const float angle, const gpcpu::float3 &axis)
{
  mElements[3] = cosf(angle/2.0f);
  float s = sinf(angle/2.0f);
  gpcpu::float3 unit = axis.normalize();
  mElements[0] = s * unit[0];
  mElements[1] = s * unit[1];
  mElements[2] = s * unit[2];
} // end Quaternion::Quaternion()

gpcpu::float4x4 Quaternion
  ::getRotationMatrix(void) const
{
  const Quaternion &q = *this;

  gpcpu::float4x4 result;
  float n=norm2();
  ...
  ::operator*(const Quaternion &rhs) const
{
  ...
  gpcpu::float3 vLhs(lhs[0], lhs[1], lhs[2]);
  gpcpu::float3 vRhs(rhs[0], rhs[1], rhs[2]);

  float ts=lhs[3]*rhs[3]-vLhs.dot(vRhs);

  gpcpu::float3 tv=lhs[3]*vRhs + rhs[3]*vLhs + vLhs.cross(vRhs);
